...
CoverTree<val_t>::CoverTree() //,bool nat = false)
	: _root_idx(-1) , _root_level(0) , _elem_count(0) ,
	_total_count(0) , _dims(1) , _nat(false) {}

template <typename val_t>
CoverTree<val_t>::CoverTree(int dims) //,bool nat = false)
	:  _root_idx(-1) , _root_level(0) , _elem_count(0) ,
	_total_count(0) , _dims(dims) , _nat(false) {}

		//nat fixed to false because rebalance is bugged
template <typename val_t>
CoverTree<val_t>::CoverTree(CoverTreeNode<val_t> n,int dims) //,bool nat = false)
	:  _root_idx(-1) , _root_level(0) , _elem_count(1) , _dims(dims) , _nat(false)
{
	_nodes.push_back(n);
...
template <typename val_t>
double CoverTree<val_t>::dist(const CoverTreeNode<val_t> & n1, const CoverTreeNode<val_t> & n2)
{
...
template <typename val_t>
double CoverTree<val_t>::sepdist(int level)
{
...
template <typename val_t>
int
CoverTree<val_t>::popLeaf(CoverTreeNode<val_t> & n)
{
...
template <typename val_t>
void CoverTree<val_t>::insert(DVec pos,val_t value)
{
	insert(CoverTreeNode<val_t>(pos,value));
}

template <typename val_t>
void CoverTree<val_t>::insert(CoverTreeNode<val_t> x)
{
	if (x.pos.size() != _dims)
...
void CoverTree<val_t>::insert(int node_idx, int & p_id, int level)
{
	...
	if (_nat)
		//insert_ord(x,p,level);
		throw RuntimeException(TRACE_INFO,"CoverTreeNode::insert: insert_ord not implemented.");
	else
                                  ...
                                  int level)
{
	...
	//std::cout << "insert_ push_back" << n.p[0] << "," << x[0] << "\n";
	//We have to first add the child because adding x to _nodes
	//might invalidate our reference to p
	p.children.push_back(node_idx);
}

#if 0
template <typename val_t>
void CoverTree<val_t>::insert_ord(CoverTreeNode<val_t> & x,
                                  CoverTreeNode<val_t> *& p,
                                  int level)
{
	if (dist(x,p) == 0)
	...
	auto cmp = [x](const CoverTreeNode<val_t> & e1,const CoverTreeNode<val_t> & e2) -> bool
				{
				   return dist(e1,x) < dist(e2,x);
				};
	std::sort(p.children.begin(),p.children.end(),cmp);
	for (auto & q : p.children)
	{
		if (dist(x,q) < covdist(level-1))
		{
			insert_ord(x,q,level-1);
			return;
		}
	}
	rebalance(p,x,level);
...
template <typename val_t>
void CoverTree<val_t>::rebalance(CoverTreeNode<val_t> & p, CoverTreeNode<val_t> & x, int level)
{
	CTNSeq<val_t> moveset;
	CTNSeq<val_t> stayset;
	for (auto q = p.children.begin(); q != p.children.end(); q++)
	{
		rebalance_rec(p,p,q,x,moveset,stayset,level);
		for (auto r : moveset)
			insert(x,r,level-1);
	}
...
template <typename val_t>
void
CoverTree<val_t>::rebalance_rec(CoverTreeNode<val_t> & p,
                                CoverTreeNode<val_t> & pq,
								typename CTNSeq<val_t>::iterator & q,
								CoverTreeNode<val_t> & x,
								CTNSeq<val_t> & moveset, CTNSeq<val_t> & stayset,
								int level)
{
	//If q is further away from p then from x
	if (dist(p,*q) > dist(*q,x))
	{
		//Then add it to the moveset (will be inserted below x)
		moveset.push_back(*q);
		_elem_count--;
		//Check same for all Descendants
		for (auto & r : descendants(*q))
		{
		...
		//Remove q from it's parent pq
		q = pq.children.erase(q)-1;
		return;
	}
	else
	{
		for (auto r = q->children.begin(); r != q->children.end(); r++)
			rebalance_rec(p,*q,r,x,moveset,stayset,level-1);

		CTNSeq<val_t> stayset_remain;
		for (auto & r : stayset)
		{
			if (dist(r,*q) <= covdist(level - 1))
				insert(r,*q,level-1);
			else
				stayset_remain.push_back(r);
		}
		stayset = stayset_remain;
		return;
	}
}
...
template <typename val_t>
val_t CoverTree<val_t>::get(DVec pos) const
{
...
std::string CoverTree<val_t>::to_string() const
{
	...
	while (true)
	{
		...
		{
			auto & n = _nodes[n_id];
			ss << ::to_string(n.pos) << ": " << n.value << std::endl;
