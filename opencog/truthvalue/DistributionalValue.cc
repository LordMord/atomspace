...
DistributionalValue::DistributionalValue(AtomSpace* as,double mode,double conf)
    : ProtoAtom(DISTRIBUTIONAL_VALUE)
{
    confidence_t cf = std::min(conf, 0.9999998);
    double count = (DEFAULT_K * cf / (1.0 - cf));
    Handle h = as->add_node(NUMBER_NODE,std::to_string(mode));
    value[h] = count;
}
...
DistributionalValuePtr DistributionalValue::TRUE_TV(AtomSpace* as)
{
    Handle h1 = as->add_node(NUMBER_NODE,"1");
    Handle h2 = as->add_node(NUMBER_NODE,"0");
    HandleCounter hc;
    hc[h1] = 1;
    hc[h2] = 0;
    return std::make_shared<const DistributionalValue>(hc);
}
DistributionalValuePtr DistributionalValue::FALSE_TV(AtomSpace* as)
{
    Handle h1 = as->add_node(NUMBER_NODE,"1");
    Handle h2 = as->add_node(NUMBER_NODE,"0");
    HandleCounter hc;
...
DistributionalValuePtr DistributionalValue::DEFAULT_TV(AtomSpace* as)
{
    Handle h1 = as->add_node(NUMBER_NODE,"1");
    Handle h2 = as->add_node(NUMBER_NODE,"0");
    HandleCounter hc;
